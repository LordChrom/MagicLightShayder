...
const float gatewayspeed = 0.0075;
...
vec3 singleEndGatewayLayer(vec2 srcPos, float timeShift, vec3 color, float depth){
    srcPos=fract(vec2(srcPos.x,srcPos.y+timeShift/depth)*vec2(depth*0.9,depth));
...
#define pos0 srcPos
    ...
    #define pos180 srcPos
#endif
    float timeShift = loopingAnimation(gatewayspeed);
