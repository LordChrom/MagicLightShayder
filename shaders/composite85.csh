...
void flushBuffer(){
    ...
    imageStore(colorimg14,ivec2(gl_LocalInvocationID.xy+gl_WorkGroupID.xy*SIZE),vec4(value,0)/DOF_STORAGE_SCALE);
}
#else
void flushBuffer(){
    uvec3 value = uvec3(0);
...
void main(){
    ...
    for(;id>=0;id-=SIZE*SIZE){
        ...
        vec4 radii = abs(textureGather(colortex12,texcoord,1));
        ...
        for(int i=0;i<4;i++){
            if(radii[i]<=0.5)
                radii[i]=r[i]=g[i]=b[i]=0;
            else
                validRadii++;
        }
        radius = (0.5/validRadii)*((radii.x+radii.y)+(radii.z+radii.w));
        #else
        ...
        #ifdef HALF_RES_DOF
        color = uvec3(round(DOF_STORAGE_SCALE*vec3((r.x+r.y)+(r.z+r.w),(g.x+g.y)+(g.z+g.w),(b.x+b.y)+(b.z+b.w)))/validRadii);
        if(radius<=0.25){
            continue;
        }
        #else
        color = uvec3(texelFetch(colortex0,samplePos + ivec2(gl_WorkGroupID.xy*SIZE),0).rgb*DOF_STORAGE_SCALE);
        if(radius<=0.5){
